...
class ArticleSummary(BaseModel):
    article_title: str = Field(..., description="Final human-readable title")
    paragraph: str     = Field(..., description="Single concise paragraph about the article")
...
def _summarize_once(article: dict) -> dict:
    structured = _get_model().with_structured_output(ArticleSummary)
    prompt = (
        "Summarize the webpage into ONE compact, neutral paragraph.\n"
...
def summarize_long_parallel(
    ...
    """
    Summarize multiple pages in parallel with:
      - per-item timeout
      - overall time budget
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max(1, workers)) as ex:
        ...
        while pending:
            if stop_event.is_set():
                if verbose: print("    Cancelled — stopping summaries.", flush=True)
                break

            if time_budget_seconds is not None and (time.time() - start) >= time_budget_seconds:
                if verbose: print("    Budget exhausted — stopping summaries.", flush=True)
                break

            # short wait so Ctrl+C is responsive
            ...
            for fut in list(done):
                ...
                try:
                    ...
                    # Note: the underlying call already ran; result() here is immediate.
                    res = fut.result()
                    results.append(res)
                except concurrent.futures.TimeoutError:
                    if verbose: print("      Timed out (skipping).", flush=True)
                except Exception as e:
                    if verbose: print(f"      Failed: {e}", flush=True)

            # rebuild pending map (drop completed)
            pending = {f: pending[f] for f in not_done}

            # also enforce per-item timeout by cancelling slow tasks opportunistically
            # (ThreadPoolExecutor can't truly cancel running work; bounded by API timeouts)
